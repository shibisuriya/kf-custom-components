...
import { initialSnakesState, GRID_MAP, initialFoodState } from './computed.js'
import { generateRandomNumber } from './utils.js'
import { generateKey, isCellValid, whichFoodToSpawn } from './helpers.js'
import {
    SNAKE_COLLIDED_WITH_WALL,
    SNAKE_SUCIDE,
    SNAKE_BODY_COLLISION,
    SNAKE_HEAD_COLLISION,
} from './errors.js'
...
class Grid {
    ...
    addSnakeToTrack(trackId, snakeId) {
        if (trackId in this.tracks) {
            Object.assign(this.tracks[trackId], {
                [snakeId]: this.snakes[snakeId],
            })
        } else {
    ...
    removeSnakeFromTrack({ trackId, snakeId }) {
        if (trackId) {
            ...
            throw new Error(
                'The snake you requested to remove does not belong to any track!'
            )
        }
    }
    ...
    initializeSnakes() {
        ...
        for (const [snakeId, initialSnakeState] of Object.entries(
            initialSnakesState
        )) {
            ...
            snake.die = (causeOfDeath) => {
                ...
                if (
                    causeOfDeath === SNAKE_COLLIDED_WITH_WALL ||
                    causeOfDeath === SNAKE_SUCIDE
                ) {
                    ...
                    throw new Error(
                        'Cause of death of snake unknown, unable to convert snakes body into food.'
                    )
                }
            }
            snake.changeSpeed = (trackId) =>
                this.switchSnakeTrack.bind(this)({ snakeId, trackId })
            ...
            snake.grid = {
                ...
                getCellsOccupiedBySnakes:
                    this.getCellsOccupiedBySnakes.bind(this),
            }
    ...
    moveSnakes(snakes) {
        ...
        const idleSnakesHash = Object.entries(this.snakes).reduce(
            (hash, [snakeId, snake]) => {
        ...
        for (let i = 0; i < movedSnakes.length; i++) {
            const [
                snakeOneId,
                { headKey: snakeOneHeadKey, hash: snakeOneHash },
            ] = movedSnakes[i]
            for (let j = i + 1; j < movedSnakes.length; j++) {
                const [
                    snakeTwoId,
                    { headKey: snakeTwoHeadKey, hash: snakeTwoHash },
                ] = movedSnakes[j]
        ...
        if (
            !('player' in this.snakes || Object.keys(this.snakes).length <= 1)
        ) {
            this.endGame()
        }
    }
    ...
    attachTickers() {
        if (!this.timers || this.timers?.length <= 0) {
            ...
            throw new Error(
                'Timers already exists, you are trying to attach timers again?'
            )
        }
    ...
    updateView() {
        if (this.viewUpdater) {
            ...
            console.warn(
                'Grid instance was not supplied a method to update the view...'
            )
        }
        ...
        if (this.updateSnakeList) {
            this.updateSnakeList(
                Object.entries(this.snakes).reduce(
    ...
    addFoodToGrid(x, y, foodType) {
        ...
        if (!(key in this.getCellsOccupiedBySnakes()) && !(key in this.food)) {
            ...
            throw new Error(
                'Trying to spawn a food in a cell that is occupied by either a snake or a food.'
            )
        }
    }
    ...
    removeFoodFromGrid(x, y) {
        if (this.isFoodCell(x, y)) {
            ...
            throw new Error(
                `Unable to remove food, since there is no food at ${x}-${y}.`
            )
        }
    }
        ...
        return Object.values(this.snakes).reduce((cells, snake) => {
            ...
            for (const [key, value] of Object.entries(hash)) {
                if (!(key in cells) && !this.isFoodCell(value.x, value.y)) {
                    ...
                    throw new Error(
                        'Two snakes or food are occupying a single cell!'
                    )
                }
            }
            return cells
    ...
    spawnFood() {
        ...
        if (keys.length > 0) {
            const randomEmptyCell =
                emptyCells[keys[generateRandomNumber(keys.length)]]
    ...
    getAnnotationData() {
        const annotationData = Object.values(this.bots).reduce(
            (annotationData, bot) => {
