import { generateKey, getOppositeDirection, isCellValid } from './helpers.js'

import { DIRECTIONS, FOOD_EFFECTS, FOOD_TYPES } from './constants.js'
import { SNAKE_COLLIDED_WITH_WALL, SNAKE_SUCIDE } from './errors.js'
...
class Snake {
    ...
    growFromBehind(units = 1) {
        for (let i = 1; i <= units; i++) {
            ...
            if (x1 - x2 === 1 && y2 - y1 === 0) {
                ...
                throw new Error(
                    "Snake's data is corrupt!, unable to find the direction."
                )
            }
            ...
            if (this.grid.isFoodCell(newTail.x, newTail.y)) {
                ...
                isCellValid(newTail.x, newTail.y) &&
                !(newTailKey in this.grid.getCellsOccupiedBySnakes())
            ) {
                addTail()
            } else {
    ...
    getPenultimateCell() {
        if (this.keys.length >= 2) {
            ...
            throw new Error(
                "The snake has only a head! it doesn't even have a neck."
            )
        }
    }
    ...
    getTail() {
        if (this.keys.length > 0) {
            ...
            throw new Error(
                "The snake doesn't have a body, so unable to select the tailKey"
            )
        }
    }
    ...
    changeDirection(direction) {
        ...
        if (direction === this.direction) {
            console.warn(
                `Snake is already moving in the ${direction} direction.`
            )
            ...
            console.warn(
                `You are trying to put the neck into a state where it will collide with its own head...`
            )
    ...
    getAnnotations() {
        if (this.isBot) {
            ...
            throw new Error(
                "This snake is not a bot, can't get any annotations data..."
            )
        }
    }
    ...
    updateAnnotations(annotations) {
        if (this.isBot) {
            ...
            throw new Error(
                'Trying to add annotations for a player that is not a bot?'
            )
        }
    }
